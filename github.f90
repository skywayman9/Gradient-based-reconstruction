...
! ***********v******************************************************************
	...
	double precision	:: vd1(-2:3,n_eqn),char_ddx(-2:2)
    double precision 	:: lefteigen(4,4), righteigen(4,4)
    double precision    :: ul(n_eqn),ur(n_eqn)

    double precision	:: a(-3:NS+3), b(-3:NS+3), c1(-3:NS+3), r(-3:NS+3),du_dx(-3:NS+3,n_eqn)
	...
	lx = -my ; ly = mx;
        ...
        do k = 1,n_eqn
            ...
            b(i) = 1.0d0
            c1(i) = 2.0d0
            r(i) = (-5.0d0*un(i,k) + 4.0d0*un(i+1,k) + un(i+2,k))/(2.0d0)

            i=-2
            a(i) = 1.0d0/4.0d0
            b(i) = 1.0d0
            c1(i) = 1.0d0/4.0d0
            r(i) = (3.0d0/4.0d0)*(un(i+1,k)-un(i-1,k))
            ...
            do i=-1,NS+1
	            ...
	            b(i) = 1.0d0
	            c1(i) = 5.0d0/14.0d0
	            r(i) = ((1.0d0/3.0d0)*((5.0d0/14.0d0)+2.0d0))*(un(i+1,k)-un(i-1,k))+((1.0d0/12.0d0)*((20.0d0/14.0d0)-1.0d0))*(un(i+2,k)-un(i-2,k))
            end do
			...
			call tridiag(a,b,c1,r,du_dx(-3:NS+3,k),NS+7)
	...
	do ix =0,NS
       ...
       do i=-2,3
			...
			vd1(i,:) = matmul(lefteigen,du_dx(i+ix,:))
		...
		do i=1,n_eqn
				...
				VOR = charstencil(0)+(1.0d0/2.0d0)*char_ddx(0)+&
								   ((-4.0d0+2.0d0)*0.25d0*(char_ddx(+1)-char_ddx(-1)) + &
								   (4.0d0*0.5d0) *(charstencil(-1)-2.0d0*charstencil(+0) +charstencil(+1)))/12.0d0
				...
				if ((VOR-charstencil(0))*(VOR-VMP)<EPSM ) then
 				...
 				DJM1 = ((-alpha+2.0d0)*0.25d0*(char_ddx(+0)-char_ddx(-2)) + (alpha*0.5d0) *(charstencil(-2)-2.0d0*charstencil(-1) +charstencil(+0)))
				DJ   = ((-alpha+2.0d0)*0.25d0*(char_ddx(+1)-char_ddx(-1)) + (alpha*0.5d0) *(charstencil(-1)-2.0d0*charstencil(+0) +charstencil(+1)))
			    DJP1 = ((-alpha+2.0d0)*0.25d0*(char_ddx(+2)-char_ddx(+0)) + (alpha*0.5d0) *(charstencil(+0)-2.0d0*charstencil(+1) +charstencil(+2)))
				   ...
				   VMIN = DMAX1(DMIN1(charstencil(0),charstencil(1),VMD),DMIN1(charstencil(0),VUL,VLC))
				   VMAX = DMIN1(DMAX1(charstencil(0),charstencil(1),VMD),DMAX1(charstencil(0),VUL,VLC))
